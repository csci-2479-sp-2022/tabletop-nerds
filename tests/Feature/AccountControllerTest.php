...
class AccountControllerTest extends TestCase
{
    ...
    public function test_get_wishlist_returns_list(): void {
        ...
        $response->assertViewHas('wishlist', $this->wishlist);
    }
    ...
    public function test_get_wishlist_returns_single_item_id(): void {
        ...
        // assert
        $response->assertStatus(200);
        $response->assertViewHas('wishlist', $macbook);
    }

    public function test_get_wishlist_invalid_id() {
        $falseId = 5;
        $this->accountServiceSpy->shouldReceive('getWishById')
            ->with($falseId)
            ->andReturn(null);
        $response = $this->get('/wishlist/' . $falseId);
